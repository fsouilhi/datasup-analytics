...
def _nettoyer(df, annee):
    ...
    for col in ("capacite","nb_voeux","nb_admis","taux_acces","pct_femmes","pct_boursiers","pct_mention_tb"):
        df[col] = pd.to_numeric(df[col], errors="coerce")
    for col in ("nom_etab","region","academie","ville","filiere_agregee","filiere_detaillee","dept_lib"):
        df[col] = df[col].astype(str).str.strip().replace({"nan": None, "": None})
    return df.reset_index(drop=True)

def _type_etab(nom, fili):
    n, f = (nom or "").lower(), (fili or "").lower()
    if "iut" in n or "i.u.t" in n or "but" in f: return "IUT"
    if "bts" in f: return "BTS"
    if "cpge" in f or "prepa" in n: return "CPGE"
    if "universite" in n or "univ" in n or "université" in n: return "Université"
    return "Autre"

def _niveau(f):
    f = (f or "").lower()
    if "master" in f: return "Master"
    if "licence pro" in f: return "Licence Pro"
    if "licence" in f: return "Licence"
    if "but" in f: return "BUT"
    if "bts" in f: return "BTS"
    if "cpge" in f: return "CPGE"
    return "Autre"

def _domaine(f, md):
    f = (f or "").lower()
    if "info" in f or "miage" in f: return md.get("INFO")
    if "droit" in f: return md.get("DROIT")
    if "gestion" in f or "management" in f: return md.get("ECO-GEST")
    if "sante" in f or "medecine" in f or "santé" in f: return md.get("SANTE")
    if "lettr" in f or "langue" in f: return md.get("LETTR")
    if "psycho" in f or "socio" in f: return md.get("SHS")
    if "math" in f: return md.get("MATH")
    if "physique" in f or "chimie" in f: return md.get("PHYS-CHIM")
    if "biolog" in f: return md.get("BIO")
    if "ingenieur" in f or "industriel" in f or "ingénieur" in f: return md.get("SCI-ING")
    return md.get("AUTRE")

def charger_parcoursup(annee):
    chemin = DOSSIER_BRUT / f"parcoursup_{annee}.csv"
    if not chemin.exists():
        journal.error("Fichier absent : %s", chemin); return
    ...
    with moteur.begin() as conn:
        row = conn.execute(text("SELECT id_campagne FROM campagne WHERE annee=:a"), {"a":annee}).fetchone()
        if not row: journal.error("Campagne %d absente", annee); return
        id_campagne = row.id_campagne
        md = {r.code: r.id_domaine for r in conn.execute(text("SELECT code,id_domaine FROM domaine"))}
        for _, e in df[["uai","nom_etab","ville","dept_lib","region","academie","filiere_agregee"]].drop_duplicates("uai").iterrows():
            uai = str(e["uai"])
            if len(uai) > 8: continue
            conn.execute(text("""
        ...
        for _, f in df[["uai","filiere_detaillee","filiere_agregee"]].drop_duplicates(["uai","filiere_detaillee"]).iterrows():
            ie = mu.get(f["uai"])
            if not ie: continue
        ...
        for _, ligne in df.iterrows():
            ...
            try:
                ...
                journal.debug("Ligne ignoree : %s", ex); nb_ko += 1
